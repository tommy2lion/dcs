...
out=$($EXE circuits/02-not-gate.dcs --input "a=0")
check "not(0)" "a=0 -> y=1" "$out"

out=$($EXE circuits/02-not-gate.dcs --input "a=1")
check "not(1)" "a=1 -> y=0" "$out"

# 2. AND gate, single step (multiple-assignment format)
out=$($EXE circuits/01-and-gate.dcs --input "a=0,b=0")
check "and(0,0)" "a=0 b=0 -> y=0" "$out"

out=$($EXE circuits/01-and-gate.dcs --input "a=1,b=0")
check "and(1,0)" "a=1 b=0 -> y=0" "$out"

out=$($EXE circuits/01-and-gate.dcs --input "a=1,b=1")
check "and(1,1)" "a=1 b=1 -> y=1" "$out"

# 3. AND gate, single step (separate --input flags)
out=$($EXE circuits/01-and-gate.dcs --input "a=0" --input "b=1")
...
expected="step  a  b  sum  carry
   ...
   2  0  1    1      0
   3  1  1    0      1"
out=$($EXE circuits/03-half-adder.dcs --input "a=0,1,0,1" --input "b=0,0,1,1")
check "half_adder 4 steps" "$expected" "$out"

# 5. No --input flags → all undefined, output is X
out=$($EXE circuits/01-and-gate.dcs)
