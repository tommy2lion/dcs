...
**[docs/step3-implementation-plan.md](docs/step3-implementation-plan.md)** — Step 3 implementation plan: every U-* carry-over sequenced into 13 stages for v1.0.0 ("teacher-demo ready") and a forward roadmap for v1.1.0 → v2.1+. Each stage is sized to fit one AI session without context drift.

↓

**[docs/step3-1.0.0-initial-design.md](docs/step3-1.0.0-initial-design.md)** — Initial design for the v1.0.0 portion of the implementation plan: code-quality changes, auto-layout + routing overhaul, shape DSL with ANSI/IEEE primitives, and the demo-circuit gallery. Pure-documentation and trivial-define stages are excluded.

↓

**[docs/step3-1.0.0-implementation-summary.md](docs/step3-1.0.0-implementation-summary.md)** — Retrospective on Stages 1–8.5 of the v1.0.0 work: what shipped (518 → 931 tests; 3 → 7 primitive gates), where the plan bent under real-world contact (the XOR-routing saga, OR-shape visual iteration), trade-offs settled, and the U-46/U-52 requirements that surfaced mid-stream and got allocated to later versions.
